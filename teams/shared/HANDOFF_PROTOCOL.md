...
7. `done_criteria`
8. `timestamp_utc`
9. `optional_team_wake_reason` (required if waking Green/Black/Grey in `lite`)
10. `optional_team_skip_risk_note` (required if skipping Green/Black/Grey in `lite`)
11. `pipeline_mode` must match the latest mode declaration for `run_id` in `teams/shared/run_mode_registry.csv`.
...
5. In `lite` mode, skipping Green/Black/Grey without a risk note invalidates handoff.

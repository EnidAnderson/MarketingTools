...
use tokio::time::{sleep, Duration}; // Import reqwest client
...
static LAST_RESET_TIME: Lazy<std::sync::Mutex<std::time::Instant>> =
...
pub async fn send_text_prompt(prompt: &str) -> Result<String, Box<dyn Error + Send + Sync>> {
    ...
    loop {
        ...
        if current_calls >= MAX_LLM_CALLS_PER_MINUTE {
            ...
            println!(
                "Rate limit hit. Waiting for {:?} before next LLM call.",
                time_to_wait
            );
        ...
        } else if current_calls > 0 {
            // Apply min delay after the first call
    ...
    if let Some(candidate) = response["candidates"].as_array().and_then(|arr| arr.get(0)) {
        if let Some(part) = candidate["content"]["parts"]
            .as_array()
